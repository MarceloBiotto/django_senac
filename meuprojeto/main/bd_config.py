...
def conecta_no_banco_de_dados():
    ...
    if num_results > 0:
        print('O banco de dados aulaDjango existe e esta pronto para uso.')
    else:
        ...
        # Criar o banco de dados aulaDjango
        cursor = cnx.cursor()
        cursor.execute('CREATE DATABASE aulaDjango;')
        cnx.commit()

    # Conectar-se ao banco de dados aulaDjango recém-criado
        cnx = mysql.connector.connect(
        host='127.0.0.1',
        user='root',
        password='',
        database='aulaDjango'  # Especificar o banco de dados
        )
